...
def process_query_sysu(data_path, mode='all', relabel=False):
    ...
    # elif mode =='indoor':
    #     ir_cameras = ['cam3', 'cam6']
    ir_cameras = ['camera4', 'camera5', 'camera6']

    file_path = os.path.join(data_path, 'npy/test_id.npy')
    files_ir = []
    ids = np.load(file_path, allow_pickle=True).tolist()
    ...
    for img_path in files_ir:
        camid, pid = int(img_path.split('/')[-3][-1]), int(img_path.split('/')[-2])
...
def process_gallery_sysu(data_path, mode='all', trial=0, relabel=False, gall_mode='single'):
    ...
    rgb_cameras = ['camera1', 'camera2', 'camera3']
    ...
    for img_path in files_rgb:
        ...
        if gall_mode == 'multi':
            for i in img_path:
                camid, pid = int(i.split('/')[-3][-1]), int(i.split('/')[-2])
                gall_img.append(i)
                gall_id.append(pid)
                gall_cam.append(camid)
