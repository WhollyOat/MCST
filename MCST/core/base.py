...
from tools import os_walk, CrossEntropyLabelSmooth, SupConLoss, TripletLoss_WRT, T2TLoss, I2T_TripletLoss
...
class Base:
    def __init__(self, config):
        ...
        self.stage0_train_epochs = config.stage0_train_epochs

        self.margin_it = config.margin_it
        self.margin_tt = config.margin_tt
    ...
    def _init_creiteron(self):
        ...
        self.i2t_tri_creiteron = I2T_TripletLoss(margin= self.margin_it)
        self.t2t_creiteron = T2TLoss(margin = self.margin_tt)
       

    def _init_optimizer_stage0(self):
        params = []
        keys = []
        ...
        self.model_optimizer_stage0 = getattr(torch.optim, 'Adam')(params)
        self.model_lr_scheduler_stage0 = create_scheduler(self.model_optimizer_stage0,
                                                 num_epochs=self.stage0_train_epochs, lr_min=self.stage1_lr_min,
    ...
    def _init_optimizer_stage4(self):
        params = []
        keys = []
        for key, value in self.model.named_parameters():
            if 'prompt_learner1' in key:
        ...
        self.model_optimizer_stage4 = getattr(torch.optim, 'Adam')(params)
        self.model_lr_scheduler_stage4 = WarmupMultiStepLR(self.model_optimizer_stage4, self.milestones,
                                             gamma=0.1, warmup_factor=0.01, warmup_iters=10)
        
    def _init_optimizer_stage5(self):
        params = []
        keys = []
        for key, value in self.model.named_parameters():
            if 'prompt_learner1' in key:
        ...
        self.model_optimizer_stage5 = getattr(torch.optim, 'Adam')(params)
        self.model_lr_scheduler_stage5 = WarmupMultiStepLR(self.model_optimizer_stage5, self.milestones,
                                             gamma=0.1, warmup_factor=0.01, warmup_iters=10)
    
    def _init_optimizer_stage6(self):
        params = []
        keys = []
        for key, value in self.model.named_parameters():
            if 'prompt_learner1' in key:
        ...
        self.model_optimizer_stage6 = getattr(torch.optim, 'Adam')(params)
        self.model_lr_scheduler_stage6 = WarmupMultiStepLR(self.model_optimizer_stage6, self.milestones,
                                             gamma=0.1, warmup_factor=0.01, warmup_iters=10)
